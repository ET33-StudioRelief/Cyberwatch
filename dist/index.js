...
(() => {
  ...
  gsapWithCSS.defaults({ ease: "power2.out", duration: 0.6 });

  // src/typescript/animations/parallaxBackground.ts
  function initBgParallax(selector3 = "[data-parallax-bg]") {
    if (!document.querySelector(selector3)) return;
    ...
    mm.add(
      ...
      (context3) => {
        const { isDesktop, reduceMotion } = context3.conditions ?? {};
        if (!isDesktop || reduceMotion) return;
        gsapWithCSS.utils.toArray(selector3).forEach((wrap3) => {
          const img = wrap3.querySelector("img");
          if (!img) return;
          const amount = parseFloat(wrap3.dataset.parallaxAmount ?? "") || 5;
          const speed = parseFloat(wrap3.dataset.parallaxSpeed ?? "") || 1.4;
          gsapWithCSS.fromTo(
            img,
            { yPercent: -amount },
            {
              ...
              ease: "none",
              scrollTrigger: {
                trigger: wrap3.closest("section") ?? wrap3,
                start: "top bottom",
                end: "bottom top",
                scrub: speed
              }
  ...
  // src/typescript/components/accordion.ts
  var openAccordion = (container, contentSelector) => {
  ...
  var closeAccordion = (container, contentSelector) => {
    const content = container.querySelector(contentSelector);
    if (!content) return;
    content.style.height = `${content.scrollHeight}px`;
    requestAnimationFrame(() => {
  ...
  var toggleAccordion = (container, contentSelector) => {
    if (container.classList.contains("is-open")) {
  ...
  var ACCORDION_SELECTOR = ".accordion-component";
  var ACCORDION_TRIGGER = ".accordion_show-content";
  var ACCORDION_CONTENT = ".accordion_hide-content";
  var ACCORDION_DEFAULT_ATTRIBUTE = "data-accordion-default";
  function initAccordion() {
    const accordions = document.querySelectorAll(ACCORDION_SELECTOR);
    if (!accordions.length) return;
    accordions.forEach((accordion) => {
      const trigger = accordion.querySelector(ACCORDION_TRIGGER);
      if (!trigger) return;
  ...
  // src/typescript/components/button.ts
  function initGlowOrbit(selector3 = ".button") {
    if (!document.querySelector(selector3)) return;
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    gsapWithCSS.utils.toArray(selector3).forEach((el) => {
      const angle = { value: 0 };
      const tween = gsapWithCSS.to(angle, {
        value: 360,
        duration: 2.4,
        ease: "none",
        repeat: -1,
  ...
  // src/typescript/components/dropdown.ts
  var openAccordion2 = (container, contentSelector) => {
    const content = container.querySelector(contentSelector);
    if (!content) return;
    container.classList.add("is-open");
    requestAnimationFrame(() => {
  ...
  var closeAccordion2 = (container, contentSelector) => {
    const content = container.querySelector(contentSelector);
    if (!content) return;
    content.style.height = `${content.scrollHeight}px`;
    requestAnimationFrame(() => {
  ...
  var toggleAccordion2 = (container, contentSelector, siblingSelector, wrapperSelector) => {
    if (container.classList.contains("is-open")) {
      closeAccordion2(container, contentSelector);
      return;
    }
    const wrapper = wrapperSelector ? container.closest(wrapperSelector) : container.parentElement;
    const siblings = wrapper ? wrapper.querySelectorAll(siblingSelector) : document.querySelectorAll(siblingSelector);
    siblings.forEach((sibling) => {
      if (sibling === container) return;
      if (sibling.classList.contains("is-open")) {
        closeAccordion2(sibling, contentSelector);
      }
    });
    openAccordion2(container, contentSelector);
  };
  var INFO_DROPDOWN_SELECTOR = ".faq-dropdown-component";
  var INFO_DROPDOWN_CONTENT = ".faq-dropdown_hidden-content";
  var INFO_DROPDOWN_WRAPPER = ".faq_list";
  function initInfoDropdown() {
    const dropdowns = document.querySelectorAll(INFO_DROPDOWN_SELECTOR);
  ...
  // src/typescript/components/footer.ts
  var randomBetween = (min, max) => min + Math.random() * (max - min);
  function initFooterGlow(selector3 = ".footer_glow-bg", boundsSelector = ".footer_btm-wrp") {
    const glow = document.querySelector(selector3);
  ...
  var TABLET_QUERY = "(max-width: 991px)";

  // src/typescript/components/navbar.ts
  var NAV_MENU_DESKTOP_QUERY = "(min-width: 1350px)";
  var SCROLL_LOCK_CLASS = "nav-scroll-lock";
  var DROPDOWN_VIEWPORT_MARGIN = 16;
  var TOGGLE_ANCHORED_DROPDOWN_CLASS = "is-wip";
  var SCROLL_THRESHOLD = 5;
  var SCROLL_DELTA = 5;
  var SCROLLED_THRESHOLD = 80;
  var positionDropdownList = (toggle, list) => {
    ...
    const overflowRight = list.getBoundingClientRect().right - (window.innerWidth - DROPDOWN_VIEWPORT_MARGIN);
    if (overflowRight > 0) {
      list.style.left = `${toggleRect.left - overflowRight}px`;
    }
  };
  var resetDropdownListPosition = (list) => {
    list.style.position = "";
    list.style.top = "";
    list.style.left = "";
    list.style.right = "";
  ...
  function initNavbar(selector3 = '[trigger="navbar"]') {
    const navbar = document.querySelector(selector3);
    if (!navbar) return;
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    let isHidden = false;
    let lastScrollY = window.scrollY;
    gsapWithCSS.set(navbar, { yPercent: 0 });
    const yTo = gsapWithCSS.quickTo(navbar, "yPercent", {
      duration: 0.3,
      ease: "easeInOut",
      overwrite: "auto"
    });
  ...
  function initNavMenu(selector3 = "[data-nav]") {
    const nav = document.querySelector(selector3);
    if (!nav) return;
  ...
  function initDesktopDropdownHover(selector3 = '[trigger="navbar"]') {
    const navbar = document.querySelector(selector3);
    if (!navbar) return;
    const desktop = window.matchMedia(NAV_MENU_DESKTOP_QUERY);
    navbar.querySelectorAll(".w-dropdown").forEach((dropdown) => {
  ...
  // node_modules/.pnpm/swiper@14.0.5/node_modules/swiper/shared/utils.mjs
  ...
  // node_modules/.pnpm/swiper@14.0.5/node_modules/swiper/shared/swiper-core.mjs
  var supportCached;
  ...
  var Observer2 = ({ swiper, extendParams, on }) => {
    const observers = [];
  ...
  // node_modules/.pnpm/swiper@14.0.5/node_modules/swiper/shared/create-element-if-not-defined.mjs
  ...
  // node_modules/.pnpm/swiper@14.0.5/node_modules/swiper/modules/navigation.mjs
  ...
  // node_modules/.pnpm/swiper@14.0.5/node_modules/swiper/shared/classes-to-selector.mjs
  function classesToSelector(classes2 = "") {
    return `.${classes2.trim().replace(/([.:!+/()[\]#>~*^$|=,'"@{}\\])/g, "\\$1").replace(/ /g, ".")}`;
  }

  // node_modules/.pnpm/swiper@14.0.5/node_modules/swiper/modules/pagination.mjs
  ...
  // src/typescript/sliders/slider-hp-cases.ts
  function initHpCasesSlider(selector3 = ".hp-cases_layout") {
    const container = document.querySelector(selector3);
    if (!container) return;
    const prevEl = container.querySelector('[trigger="cases-prev-slide"]');
    const nextEl = container.querySelector('[trigger="cases-next-slide"]');
    const paginationEl = container.querySelector('[trigger="cases-pagination"]');
  ...
  // src/typescript/sliders/slider-hp-steps.ts
  var BREAKPOINT_QUERY = "(max-width: 1350px)";
  function initHpStepsSlider(selector3 = ".hp-steps_layout") {
    const container = document.querySelector(selector3);
    if (!container) return;
    const scope = container.parentElement ?? container;
    const prevEl = scope.querySelector('[trigger="hp-steps-prev-slide"]');
    const nextEl = scope.querySelector('[trigger="hp-steps-next-slide"]');
    const paginationEl = scope.querySelector('[trigger="hp-steps-pagination"]');
  ...
  // src/typescript/sliders/slider-industries.ts
  function initIndustriesSlider(selector3 = ".hp-industries_layout") {
    const container = document.querySelector(selector3);
    if (!container) return;
    const prevEl = container.querySelector('[trigger="industries-prev-slide"]');
    const nextEl = container.querySelector('[trigger="industries-next-slide"]');
    const paginationEl = container.querySelector('[trigger="industries-pagination"]');
    new Swiper(container, {
      slidesPerView: "auto",
      spaceBetween: 40,
      rewind: true,
  ...
  // src/typescript/sliders/slider-testimonial.ts
  function initTestimonialSlider(selector3 = ".testimonial_layout") {
    const container = document.querySelector(selector3);
    if (!container) return;
    const scope = container.parentElement ?? container;
    const prevEl = scope.querySelector('[trigger="testimonial-prev-slide"]');
    const nextEl = scope.querySelector('[trigger="testimonial-next-slide"]');
    const paginationEl = scope.querySelector('[trigger="testimonial-pagination"]');
    new Swiper(container, {
      slidesPerView: "auto",
      centeredSlides: true,
      initialSlide: 1,
  ...
  // src/utils/marker.ts
  async function launchMarkerSDK() {
    const widget = await esm_default.loadWidget({
